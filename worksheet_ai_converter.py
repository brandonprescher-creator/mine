...
class WorksheetAIConverter:
    ...
    def analyze_math_worksheet(self, text: str, analysis: Dict) -> Dict:
        ...
        for line in lines:
            if "?" in line and len(line.split()) > 5:
                problems.append({"type": "word_problem", "text": line.strip()})
        ...
        if (
            "addition" in analysis["problem_types"]
            or "subtraction" in analysis["problem_types"]
        ):
            analysis["topic"] = "Basic Operations"
        elif (
            "multiplication" in analysis["problem_types"]
            or "division" in analysis["problem_types"]
        ):
    ...
    def analyze_science_worksheet(self, text: str, analysis: Dict) -> Dict:
        ...
        if re.search(self.science_patterns["multiple_choice"], text):
            ...
            for q in questions:
                problems.append({"type": "multiple_choice", "text": q})
        ...
        if re.search(self.science_patterns["fill_blank"], text):
            ...
            for b in blanks:
                problems.append({"type": "fill_blank", "text": b})
    ...
    def analyze_general_worksheet(self, text: str, analysis: Dict) -> Dict:
        ...
        for item in numbered_items:
            problems.append({"type": "general", "text": item})
    ...
    def generate_lesson_from_worksheet(self, analysis: Dict, subject: str) -> Dict:
        ...
        examples = self.generate_worked_examples(
            problem_types, topic, analysis["problems"]
        )
    ...
    def generate_teaching_steps(
        self, problem_types: List[str], topic: str, subject: str
    ) -> List[Dict]:
        """Generate teaching steps based on problem types"""
        steps = [
            {
    ...
    def generate_worked_examples(
        self, problem_types: List[str], topic: str, problems: List[Dict]
    ) -> List[Dict]:
    ...
    def save_to_database(
        self, lesson_data: Dict, practice_problems: List[Dict], subject: str
    ) -> int:
        ...
        subject_obj = next(
            (s for s in subjects if subject.lower() in s["name"].lower()), subjects[0]
        )
        ...
        for i, problem in enumerate(practice_problems, 1):
            add_practice_problem(
                ...
                hints=["Take your time", "Show your work", "Double-check calculations"],
