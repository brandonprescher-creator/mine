...
def add_subject(name: str, description: str = "", icon: str = "📚", display_order: int = 0) -> int:
    conn = get_connection()
    cursor = conn.cursor()
    cursor.execute(
...
def add_topic(subject_id: int, name: str, description: str = "", display_order: int = 0) -> int:
    conn = get_connection()
    cursor = conn.cursor()
    cursor.execute(
...
def add_lesson(topic_id: int, title: str, description: str, steps: List[str], 
               examples: List[Dict], source_type: str = "builtin", 
               source_file: str = None, display_order: int = 0) -> int:
    conn = get_connection()
    cursor = conn.cursor()
    cursor.execute(
        """INSERT INTO lessons (topic_id, title, description, steps, examples, 
           ...
           VALUES (?, ?, ?, ?, ?, ?, ?, ?)""",
        (topic_id, title, description, json.dumps(steps), json.dumps(examples), 
         source_type, source_file, display_order)
    )
...
def get_lessons_by_topic(topic_id: int) -> List[Dict]:
    ...
    for row in cursor.fetchall():
        ...
        lesson['examples'] = json.loads(lesson['examples']) if lesson['examples'] else []
...
def get_lesson_by_id(lesson_id: int) -> Optional[Dict]:
    ...
    if row:
        ...
        lesson['steps'] = json.loads(lesson['steps']) if lesson['steps'] else []
        lesson['examples'] = json.loads(lesson['examples']) if lesson['examples'] else []
        lesson['additional_resources'] = json.loads(lesson['additional_resources']) if lesson['additional_resources'] else []
...
def add_practice_problem(lesson_id: int, question: str, answer: str, 
                        steps: List[str], hints: List[str] = None, 
                        difficulty: str = "medium", display_order: int = 0) -> int:
    conn = get_connection()
    cursor = conn.cursor()
    cursor.execute(
        """INSERT INTO practice_problems (lesson_id, question, answer, steps, hints, difficulty, display_order) 
           VALUES (?, ?, ?, ?, ?, ?, ?)""",
        (lesson_id, question, answer, json.dumps(steps), json.dumps(hints or []), difficulty, display_order)
    )
...
def record_practice_attempt(lesson_id: int, problem_id: int, is_correct: bool):
    ...
    if existing:
        ...
        mastery = 'mastered' if score / attempts >= 0.8 and attempts >= 3 else 'practicing'
        
        cursor.execute(
            """UPDATE student_progress 
               SET attempts = ?, score = ?, last_attempted = ?, mastery_level = ?, completed = ?
               WHERE id = ?""",
            (attempts, score, datetime.now(), mastery, 1 if is_correct else 0, existing['id'])
        )
    else:
        mastery = 'practicing'
        cursor.execute(
            """INSERT INTO student_progress 
               ...
               VALUES (?, ?, 1, ?, ?, ?, ?)""",
            (lesson_id, problem_id, 1 if is_correct else 0, datetime.now(), mastery, 1 if is_correct else 0)
        )
...
# API cache functions
def cache_api_response(api_source: str, query: str, response_data: Dict, expires_hours: int = 24):
    conn = get_connection()
    cursor = conn.cursor()
...
def get_practice_problems_by_topic(topic_id: int) -> List[Dict]:
    ...
    for row in cursor.fetchall():
        ...
        problem['explanation'] = problem['hints'][0] if problem['hints'] else "Great job!"
