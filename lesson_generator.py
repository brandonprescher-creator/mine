...
def analyze_content(text: str) -> Dict:
    ...
    return {"subject": subject, "title": title, "word_count": len(text.split())}
        ...
        examples.append(
            {
                ...
                "content": (
                    paragraphs[1][:300]
                    if len(paragraphs) > 1
                    else "Practice the concepts explained above."
                ),
            }
        )
...
def generate_practice_problems(
    text: str, lesson_title: str, count: int = 5
) -> List[Dict]:
    """
