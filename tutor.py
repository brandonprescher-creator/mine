...
with st.sidebar:
    st.markdown(
        '<div class="fun-title">🎓 ULTIMATE TUTOR</div>', unsafe_allow_html=True
    )
    ...
    st.markdown(
        f"**📚 Lessons:** {progress['lessons_started']}/{progress['total_lessons']}"
    )
...
if "🏠 Home" in page:
    ...
    st.markdown(
        '<div class="fun-title">WELCOME TO ULTIMATE TUTOR!</div>',
        unsafe_allow_html=True,
    )
    ...
    with col2:
        if st.button("🎮 TRY IT NOW!", key="division_btn"):
            ...
            math_subject = next(
                (s for s in subjects if s["name"] == "Mathematics"), None
            )
            if math_subject:
                topics = get_topics_by_subject(math_subject["id"])
                division_topic = next(
                    (t for t in topics if "Division" in t["name"]), None
                )
                if division_topic:
    ...
    for idx, subject in enumerate(subjects[:6]):  # Show first 6 subjects
        with cols[idx % 3]:
            ...
            if st.button(
                f"EXPLORE {subject['name'].upper()}!",
                key=f"home_subject_{subject['id']}",
            ):
    ...
    st.markdown(
        '<div class="fun-title">🎨 PICK YOUR FAVORITE SUBJECT! 🎨</div>',
        unsafe_allow_html=True,
    )
    ...
    for idx, subject in enumerate(subjects):
        with cols[idx % 3]:
            ...
            if st.button(
                f"🚀 EXPLORE {subject['name'].upper()}!", key=f"subject_{subject['id']}"
            ):
                st.session_state.selected_subject = subject
                st.session_state.current_page = "topics"
                st.rerun()
    ...
    if st.session_state.selected_subject:
        ...
        for topic in topics:
            ...
            if st.button(
                f"🎮 START {topic['name'].upper()}!", key=f"topic_{topic['id']}"
            ):
    ...
    st.markdown(
        '<div class="fun-title">📤 UPLOAD YOUR COOL STUFF! 📤</div>',
        unsafe_allow_html=True,
    )
    ...
    if uploaded_file is not None:
        ...
        if st.button("📖 Process & Create Lesson", type="primary"):
            with st.spinner("🔄 Reading and analyzing your file..."):
                ...
                file_type, extracted_text = process_uploaded_file(
                    uploaded_file.name, file_bytes
                )

                if extracted_text:
                    # Save to database
                    file_id = add_uploaded_file(
                        uploaded_file.name, file_type, extracted_text
                    )
                    ...
                    with st.spinner("🤖 Generating lesson..."):
                        ...
                        lesson_title = st.text_input(
                            "Lesson Title", value=lesson_data["title"]
                        )
                        lesson_desc = st.text_area(
                            "Description", value=lesson_data["description"], height=100
                        )
                        ...
                        for i, step in enumerate(lesson_data["steps"], 1):
                            step_text = st.text_area(
                                f"Step {i}", value=step, height=80, key=f"step_{i}"
                            )
                        ...
                        default_idx = (
                            subject_names.index(suggested_subject)
                            if suggested_subject in subject_names
                            else 0
                        )

                        selected_subject_name = st.selectbox(
                            "Subject", subject_names, index=default_idx
                        )
                        selected_subject = next(
                            s for s in subjects if s["name"] == selected_subject_name
                        )

                        new_topic_name = st.text_input(
                            "Topic Name", value="Uploaded Content"
                        )
    ...
    st.markdown(
        '<div class="fun-title">🔍 SEARCH FOR COOL STUFF! 🔍</div>',
        unsafe_allow_html=True,
    )
    ...
    search_query = st.text_input(
        ...
        placeholder="e.g., division, fractions, dinosaurs, space...",
    )
        ...
        with st.spinner("Searching..."):
            ...
            if results:
                ...
                for lesson in results:
                    with st.expander(
                        f"📖 {lesson['title']} ({lesson['subject_name']} > {lesson['topic_name']})"
                    ):
                ...
                st.warning(
                    "No lessons found. Try different keywords or upload your own content!"
                )

elif "❓ Ask Tutor" in page:
    st.markdown(
        '<div class="fun-title">❓ ASK ME ANYTHING! ❓</div>', unsafe_allow_html=True
    )
    ...
    question = st.text_area(
        ...
        placeholder="e.g., How does photosynthesis work? What is long division? Why is the sky blue?",
        height=100,
    )
        ...
        if question:
            with st.spinner("🤔 Searching for answers..."):
                ...
                if local_results:
                    for lesson in local_results[:3]:
                        with st.expander(f"📖 {lesson['title']}"):
                            st.write(lesson["description"])
                            if st.button(
                                "View Full Lesson", key=f"ask_view_{lesson['id']}"
                            ):
                                st.session_state.selected_lesson = lesson["id"]
                                st.session_state.current_page = "lesson_view"
                                st.rerun()
                ...
                if api_results["sources"]:
                    ...
                    st.info(
                        "Try different keywords or check your internet connection for online resources."
                    )
        else:
    ...
    st.markdown(
        '<div class="fun-title">🏆 YOUR AWESOME PROGRESS! 🏆</div>',
        unsafe_allow_html=True,
    )
    ...
    if badges:
        ...
        for i, badge in enumerate(badges):
            with cols[i]:
                st.markdown(
                    f'<div class="progress-badge">{badge}</div>', unsafe_allow_html=True
                )
    else:
        st.markdown(
            """
...
if (
    ...
    or st.session_state.get("current_page") == "lessons"
):
    ...
    if lessons:
        ...
        for lesson in lessons:
            ...
            if st.button(
                f"🎮 START {lesson['title'].upper()}!", key=f"lesson_{lesson['id']}"
            ):
                st.session_state.selected_lesson = lesson["id"]
                st.session_state.view_mode = "lesson"
                st.session_state.current_page = "lesson_view"
...
if st.session_state.get("current_page") == "lesson_view" or st.session_state.get(
    "selected_lesson"
):
    ...
    if lesson:
        ...
        with col1:
            if st.button(
                "📖 LEARN",
                type=(
                    "primary" if st.session_state.view_mode == "lesson" else "secondary"
                ),
            ):
                st.session_state.view_mode = "lesson"
                st.rerun()
        ...
        if st.session_state.view_mode == "lesson":
            ...
            for i, step in enumerate(lesson["steps"], 1):
                st.markdown(
                    f'<div class="step-box"><strong>🎯 Step {i}:</strong> {step}</div>',
                    unsafe_allow_html=True,
                )
            ...
            if lesson["examples"]:
                ...
                for example in lesson["examples"]:
                    st.markdown(
                        f'<div class="example-box"><strong>🌟 {example["title"]}:</strong><br>{example["content"]}</div>',
                        unsafe_allow_html=True,
                    )
            ...
            with st.expander("🌐 More Resources (Online)"):
                ...
                try:
                    enriched = enrich_lesson_with_api_data(
                    ...
                    if enriched.get("wikipedia"):
                        ...
                        st.markdown(
                            f"**📖 Wikipedia: [{wiki['title']}]({wiki['url']})**"
                        )
                    ...
                    st.warning(
                        "Unable to load online resources. Check your internet connection."
                    )
            ...
            if problems:
                for i, problem in enumerate(problems, 1):
                    ...
                    if st.session_state.show_feedback.get(problem["id"]):
                        user_ans = (
                            st.session_state.practice_answers.get(problem["id"], "")
                            .strip()
                            .lower()
                        )
                        ...
                        if user_ans == correct_ans:
                            st.markdown(
                                '<div class="correct-answer">🎉 AWESOME! YOU GOT IT RIGHT! 🎉<br>You\'re doing GREAT! Keep it up! 🌟</div>',
                                unsafe_allow_html=True,
                            )
                        else:
                            st.markdown(
                                f'<div class="incorrect-answer">🤔 Not quite right, but that\'s OK! 🤔<br>The correct answer is: <strong>{problem["answer"]}</strong><br>Try again - you\'ve got this! 💪</div>',
                                unsafe_allow_html=True,
                            )
                ...
                if lesson_prog["total_problems"] > 0:
                    ...
                    mastery_pct = (
                        lesson_prog["mastered"] / lesson_prog["total_problems"]
                    ) * 100
                    st.progress(mastery_pct / 100)
                    st.caption(
                        f"{lesson_prog['mastered']} of {lesson_prog['total_problems']} problems mastered ({lesson_prog['avg_score']*100:.0f}% average)"
                    )
            else:
...
st.markdown("---")
st.caption(
    "🎓 Ultimate Tutoring Program | Comprehensive K-8 Education | Offline Capable"
)
