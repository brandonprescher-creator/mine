...
class WorksheetGeneratorAPI:
    ...
    def generate_math_worksheet_from_apis(self, topic: str, grade: int, count: int = 20) -> Dict:
    ...
    def generate_science_worksheet_from_apis(self, topic: str, grade: int, count: int = 20) -> Dict:
    ...
    def generate_language_worksheet_from_apis(self, topic: str, grade: int, count: int = 20) -> Dict:
    ...
    def get_periodic_table_questions(self, count: int) -> List[Dict]:
        ...
        elements = ['Hydrogen', 'Helium', 'Carbon', 'Nitrogen', 'Oxygen', 'Iron', 'Gold', 'Silver']
            ...
            if response.status_code == 200:
                ...
                for i, q in enumerate(data.get('results', [])[:count]):
                    problems.append({
                        ...
                        'options': [q.get('correct_answer')] + q.get('incorrect_answers', []),
    ...
    def get_dictionary_questions(self, count: int) -> List[Dict]:
        ...
        words = ['comprehend', 'analyze', 'synthesize', 'evaluate', 'create', 'understand', 
                'interpret', 'infer', 'summarize', 'compare']
    ...
    def get_poetry_questions(self, count: int) -> List[Dict]:
        ...
        try:
            ...
            if response.status_code == 200:
                poem = response.json()[0] if isinstance(response.json(), list) else response.json()
                problems.append({
                    'number': 1,
    ...
    def generate_grammar_problems(self, topic: str, count: int) -> List[Dict]:
        ...
        grammar_exercises = [
            {'question': 'Identify the noun in this sentence: "The dog ran quickly."', 'answer': 'dog'},
            ...
            {'question': 'Correct the sentence: "me and him went home"', 'answer': 'He and I went home'},
        ]
    ...
    def create_worksheet_pdf(self, worksheet: Dict) -> str:
        ...
        c.drawString(inch, height - 1.9*inch, f"Date: {datetime.now().strftime('%B %d, %Y')}")
        ...
        for i, problem in enumerate(worksheet['problems'][:30], 1):  # Max 30 per page
            ...
            if problem.get('type') != 'multiple_choice':
                c.drawString(inch + 0.3*inch, y_position - 0.3*inch, "Answer: ___________________")
                y_position -= 0.7*inch
            else:
                # Multiple choice options
                options = problem.get('options', [])
                for j, option in enumerate(options[:4]):
                    c.drawString(inch + 0.5*inch, y_position - (j+1)*0.25*inch, 
                               f"{chr(65+j)}) {option}")
                y_position -= inch
    ...
    def generate_comprehensive_worksheet_pack(self, grade: int, subject: str) -> List[Dict]:
        ...
        if subject.lower() == 'math':
            ...
            topics = ['Matter', 'Energy', 'Living Things', 'Earth Science', 
                     'Space', 'Forces', 'Weather', 'Ecosystems']
            ...
            topics = ['Vocabulary', 'Grammar', 'Reading Comprehension', 
...
def generate_worksheet(topic: str, subject: str, grade: int) -> Dict:
    """Quick function to generate worksheet"""
    if subject.lower() == 'math':
        return worksheet_generator_api.generate_math_worksheet_from_apis(topic, grade)
    elif subject.lower() == 'science':
        return worksheet_generator_api.generate_science_worksheet_from_apis(topic, grade)
    elif subject.lower() == 'english':
        return worksheet_generator_api.generate_language_worksheet_from_apis(topic, grade)
    else:
