...
class AllEducationAPIs:
    ...
    def met_museum_search(query: str) -> list[ArtworkData]:
        ...
        search_data = safe_get(search_url, params={'q': query, 'hasImages': 'true'}, default={})
    ...
    def pubchem_compound(name: str) -> ScientificData:
        ...
        return ScientificData(
            ...
            data_points={
                'molecular_formula': next((p.get('value', {}).get('sval') for p in props if p.get('urn', {}).get('label') == 'Molecular Formula'), None),
                'molecular_weight': next((p.get('value', {}).get('fval') for p in props if p.get('urn', {}).get('label') == 'Molecular Weight'), None)
            }
        )
