...
def mega_search(query: str):
        ...
        try:
            ...
            if result:
                if isinstance(result, list):
                    results['content'][api_name] = [r.to_dict() if hasattr(r, 'to_dict') else r for r in result]
                else:
                    results['content'][api_name] = result.to_dict() if hasattr(result, 'to_dict') else result
    ...
    results['summary'] = {
        ...
        'successful': sum(1 for v in results['content'].values() if 'error' not in str(v)),
