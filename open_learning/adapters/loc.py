...
def search(query: str, count: int = 10) -> list[ContentCard]:
    ...
    for item in data.get('results', [])[:count]:
        card = ContentCard(
            ...
            url=item.get('id', ''),
            text=item.get('description', [''])[0] if isinstance(item.get('description'), list) else item.get('description', ''),
            image=item.get('image_url', [''])[0] if isinstance(item.get('image_url'), list) else item.get('image_url'),
            meta={
                'date': item.get('date'),
