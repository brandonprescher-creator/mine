...
def check_ela_topics():
    ...
    if ela_subject:
        ...
        cursor.execute("SELECT id, name, description FROM topics WHERE subject_id = ? ORDER BY display_order", (ela_subject[0],))
        ...
        print(f"\nLesson counts:")
        for topic in topics:
            cursor.execute("SELECT COUNT(*) FROM lessons WHERE topic_id = ?", (topic[0],))
